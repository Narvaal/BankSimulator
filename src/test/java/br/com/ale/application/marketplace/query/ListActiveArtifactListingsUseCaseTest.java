...
class ListActiveArtifactListingsUseCaseTest {
  ...
  void setup() {
    ...
    useCase =
        new ListActiveArtifactListingsUseCase(accountService, artifactListingService, jwtService);
  ...
  private Artifact newArtifact(String name) {
    return artifactService.createAsset(
        new CreateArtifactRequest(Map.of("name", name, "rarity", "Common"), 10));
  }
  ...
  private ArtifactListing list(ArtifactUnit unit, Account seller, BigDecimal price) {
    return artifactListingService.createArtifactOffer(
        new CreateArtifactListingRequest(
            unit.getId(), seller.getId(), price, ArtifactListingStatus.ACTIVE));
  }

  private void buy(ArtifactListing listing, Client buyerClient) {
    purchaseUseCase.execute(
        new PurchaseArtifactCommand(
  ...
  void shouldFilterByPriceRange() {
    ...
    var filter =
        new ArtifactListingFilter(
            null, null, "newest", new BigDecimal("10.00"), new BigDecimal("100.00"));
    var result = useCase.execute(null, filter, 0, 20);
