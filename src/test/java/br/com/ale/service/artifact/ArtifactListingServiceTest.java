...
class ArtifactListingServiceTest {
    ...
    void shouldCreateArtifactOffer() {
        ...
        ArtifactListing listing = artifactListingService.createArtifactOffer(
    ...
    void shouldSelectArtifactListingById() {
        ...
        ArtifactListing created = artifactListingService.createArtifactOffer(
                new CreateArtifactListingRequest(
                        unity.getId(),
                        sellerAccountId,
        ...
        ArtifactListing found =
                artifactListingService.selectById(created.getId());
    ...
    void shouldFailWhenArtifactListingNotFound() {
        ...
        RuntimeException ex = assertThrows(
                ...
                () -> artifactListingService.selectById(9999L)
        );

        assertTrue(
                ex.getMessage().contains("Service error while selecting artifact listing")
        );
        assertNotNull(ex.getCause());
        assertTrue(
                ex.getCause().getMessage().contains("Artifact listing not found")
        );
    }
    ...
    void shouldSelectArtifactListingsByStatus() {
        ...
        assertTrue(
                listings.stream().allMatch(l -> l.getStatus() == ArtifactListingStatus.ACTIVE)
        );
    }
