...
class ArtifactGenerationServiceTest {
  ...
  private final ArtifactGenerationService assetGenerationService = new ArtifactGenerationService();
  ...
  void shouldGenerateCorrectAmountOfAssets() {
    ...
    assertEquals(ArtifactGenerationService.NUMBER_OF_ASSETS_PER_WEEK, assets.size());
  }
  ...
  void shouldNotGenerateDuplicateAssetsInSameWeek() {
    ...
    assertEquals(assets.size(), uniqueNames.size(), "Assets should be unique within the same week");
  }
  ...
  void shouldGenerateDifferentAssetsOnDifferentRuns() {
    ...
    assertNotEquals(
        extractNames(firstRun), extractNames(secondRun), "Two generations should not be identical");
  }

  private List<String> extractNames(List<Artifact> assets) {
    return assets.stream().map(Artifact::getName).toList();
  }
}
