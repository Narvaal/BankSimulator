    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      try {
        ArtifactListing listing =
            artifactListingDAO
                .selectByIdForUpdate(conn, request.listingId())
        ...
        ArtifactUnit unity =
            artifactUnitDAO
                .selectByIdForUpdate(conn, listing.getArtifactUnitId())
                .orElseThrow(() -> new RuntimeException("ArtifactUnit not found"));
        ...
        artifactListingDAO.updateStatus(conn, listing.getId(), ArtifactListingStatus.SOLD);

        ArtifactTransfer transfer =
            artifactTransferDAO.insert(
                conn,
                new CreateArtifactTransferRequest(
                    unity.getId(), listing.getSellerAccountId(), request.buyerAccountId()));

        boolean transferred =
            artifactUnitDAO.tryTransferOwnership(
                conn, unity.getId(), listing.getSellerAccountId(), request.buyerAccountId());

        if (!transferred) throw new RuntimeException("Artifact not transferred");

        conn.commit();

        ArtifactPurchase purchase =
            new ArtifactPurchase(
                listing.getId(),
                unity.getId(),
