...
public class ArtifactPriceHistoryService {
  ...
  public ArtifactPriceHistory registerPriceChange(
      long artifactListingId, BigDecimal newPrice, long changedByAccountId, ReasonType reason) {
    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      try {
        ...
        if (reason == ReasonType.SOLD) {
          oldPrice =
              artifactPriceHistoryDAO
                  .selectLatestByArtifactUnitId(conn, listing.getArtifactUnitId())
  ...
  public List<ArtifactPriceHistory> listByArtifactId(long artifactId) {
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      throw new RuntimeException(
          "Service error while listing artifact price history " + "[artifactId=" + artifactId + "]",
          e);
    }
  }
}
