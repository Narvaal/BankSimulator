...
public class EmailVerificationService {
  ...
  public long create(CreateEmailVerificationRequest request) {
    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      try {
        emailVerificationDAO.invalidatePreviousTokens(conn, request.clientId(), request.type());
  ...
  public Optional<EmailVerification> findByToken(String token, EmailVerificationType type) {
    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      throw new RuntimeException(
          "Service error while fetching email verification " + "[token=" + token + "]", e);
    }
  }

  public Optional<EmailVerification> findActiveByClientId(
      long clientId, EmailVerificationType type) {
    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      throw new RuntimeException(
          "Service error while fetching active email verification " + "[clientId=" + clientId + "]",
          e);
    }
  }
  ...
  public EmailVerification confirmToken(String token, EmailVerificationType type) {
    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      throw new RuntimeException(
          "Service error while confirming email verification " + "[token=" + token + "]", e);
    }
  }
}
