...
public class AccountService {
  ...
  public AccountService(
      ConnectionProvider connectionProvider, PrivateKeyStorage privateKeyStorage) {
  ...
  public Account getAccountByNumber(String accountNumber) {
    try (Connection conn = connectionProvider.getConnection()) {
      return accountDAO
          ...
          .orElseThrow(
              () ->
                  new RuntimeException("Account not found [accountNumber=" + accountNumber + "]"));
    } catch (Exception e) {
      throw new RuntimeException(
          "Service error while selecting account " + "[accountNumber=" + accountNumber + "]", e);
    }
  }
  ...
  public Account getAccountById(long accountId) {
    try (Connection conn = connectionProvider.getConnection()) {
      return accountDAO
          .selectById(conn, accountId)
          .orElseThrow(
              () -> new RuntimeException("Account not found [accountId=" + accountId + "]"));
    } catch (Exception e) {
      throw new RuntimeException(
          "Service error while selecting account " + "[accountId=" + accountId + "]", e);
    }
  }
  ...
  public Optional<Account> getAccountByClientId(long clientId) {
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      throw new RuntimeException(
          "Service error while selecting account " + "[clientId=" + clientId + "]", e);
    }
  }
  ...
  public AccountDetailsResponse getAccountDetailsById(long accountId) {
    try (Connection conn = connectionProvider.getConnection()) {
      return accountDAO
          .selectDetailsById(conn, accountId)
          .orElseThrow(
              () -> new RuntimeException("Account not found [accountId=" + accountId + "]"));
    } catch (Exception e) {
      throw new RuntimeException(
          "Service error while selecting account details " + "[accountId=" + accountId + "]", e);
    }
  }
  ...
  public void updateAccount(UpdateAccountRequest request) {
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      try {
        ...
        if (rows == 0) {
          throw new RuntimeException("Account not found [accountId=" + request.id() + "]");
        }
      ...
      throw new RuntimeException(
          "Service error while updating account " + "[accountId=" + request.id() + "]", e);
    }
  }
  ...
  public void transfer(long fromAccountId, long toAccountId, BigDecimal amount) {
    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      try {
                ...
                .orElseThrow(
                    () ->
                        new RuntimeException(
                            "Source account not found " + "[fromAccountId=" + fromAccountId + "]"));
        ...
        String message =
            TransactionMessageBuilder.build(fromAccountId, toAccountId, amount, timestamp);

        PrivateKey privateKey = privateKeyStorage.get(fromAccountId);
        if (privateKey == null) {
          throw new RuntimeException("Private key not found [fromAccountId=" + fromAccountId + "]");
        }
        ...
        int debited =
            accountDAO.debit(
                conn, new CreateBalanceOperationRequest(from.getAccountNumber(), amount));

        if (debited == 0) {
          transactionDAO.update(
              conn, new UpdateTransactionRequest(transactionId, TransactionStatus.FAILED));
          throw new RuntimeException("Insufficient balance [fromAccountId=" + fromAccountId + "]");
        }

        int credited =
            accountDAO.credit(
                conn, new CreateBalanceOperationRequest(to.getAccountNumber(), amount));
        ...
        transactionDAO.update(
            conn, new UpdateTransactionRequest(transactionId, TransactionStatus.COMPLETE));
  ...
  public void credit(String accountNumber, BigDecimal amount) {
    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      try {
        ...
        String message = TransactionMessageBuilder.build(null, account.getId(), amount, timestamp);

        PrivateKey privateKey = privateKeyStorage.get(account.getId());
        if (privateKey == null) {
          throw new RuntimeException("Private key not found [accountNumber=" + accountNumber + "]");
        }
        ...
        int credited =
            accountDAO.credit(conn, new CreateBalanceOperationRequest(accountNumber, amount));

        if (credited == 0) {
          transactionDAO.update(
              conn, new UpdateTransactionRequest(transactionId, TransactionStatus.FAILED));
          throw new RuntimeException(
              "Failed to credit account [accountNumber="
                  + accountNumber
                  + ", amount="
                  + amount
                  + "]");
  ...
  public PublicProfileResponse getPublicProfile(long accountId) {
    try (Connection conn = connectionProvider.getConnection()) {
      return accountDAO
          .selectPublicProfileById(conn, accountId)
  ...
  public PublicProfilePageView searchByName(String query, int page, int pageSize) {
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      throw new RuntimeException("Service error while searching accounts [query=" + query + "]", e);
    }
  }
  ...
  public void debit(String accountNumber, BigDecimal amount) {
    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      try {
        ...
        String message = TransactionMessageBuilder.build(account.getId(), null, amount, timestamp);
        ...
        int debited =
            accountDAO.debit(conn, new CreateBalanceOperationRequest(accountNumber, amount));

        if (debited == 0) {
          transactionDAO.update(
              conn, new UpdateTransactionRequest(transactionId, TransactionStatus.FAILED));
          throw new RuntimeException(
              "Insufficient balance [accountNumber=" + accountNumber + ", amount=" + amount + "]");
        }
