...
public class ArtifactUnitService {
  ...
  public boolean tryUpdateToMarket(long artifactUnitId, long accountId) {
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      throw new RuntimeException(
          "Service error while lock for market [unityId=" + artifactUnitId + "]", e);
    }
  }
  ...
  public ArtifactUnit createArtifactUnit(CreateArtifactUnitRequest request) {
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      try {
        ...
        if (updated == 0) {
          throw new RuntimeException(
              "Artifact supply not available [artifactId=" + request.artifactId() + "]");
        }
      ...
      throw new RuntimeException(
          "Service error while creating artifactUnit [artifactId="
              + request.artifactId()
              + ", ownerAccountId="
  ...
  public ArtifactUnit selectById(long artifactUnitId) {
    try (Connection conn = connectionProvider.getConnection()) {
      return artifactUnitDAO
          .selectById(conn, artifactUnitId)
          .orElseThrow(() -> new ArtifactUnitNotFoundException(artifactUnitId));
    } catch (ArtifactUnitNotFoundException e) {
      throw e;
    } catch (Exception e) {
      throw new RuntimeException(
          "Service error while selecting artifactUnit [id=" + artifactUnitId + "]", e);
    }
  }
