...
public class ArtifactListingService {
  ...
  public ArtifactListingService(ConnectionProvider connectionProvider) {
    this.connectionProvider = connectionProvider;
  }

  public ArtifactListing changePrice(
      long listingId, BigDecimal newPrice, long changedByAccountId, ReasonType reason) {
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      try {
        ArtifactListing listing =
            artifactListingDAO
                .selectByIdForUpdate(conn, listingId)
        ...
        artifactListingDAO.updatePrice(conn, listingId, newPrice);
        ...
        return artifactListingDAO.selectById(conn, listingId).orElseThrow();
  ...
  public void cancelListing(long listingId, long clientId) {
    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      try {
        ...
        ArtifactListing listing = artifactListingDAO.selectById(conn, listingId).orElseThrow();
        ...
        ArtifactUnit unity =
            artifactUnitDAO.selectById(conn, listing.getArtifactUnitId()).orElseThrow();

        Account account = accountDAO.selectById(conn, unity.getOwnerAccountId()).orElseThrow();
  ...
  public ArtifactListing createArtifactOffer(CreateArtifactListingRequest request) {
    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      try {
        ...
        boolean locked =
            artifactUnitDAO.tryUpdateToMarket(
                conn, request.artifactUnitId(), request.sellerAccountId());
  ...
  public List<ArtifactListing> selectByStatus(ArtifactListingStatus status) {
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      throw new RuntimeException(
          "Service error while selecting artifact listing " + "[status=" + status.name() + "]", e);
    }
  }
  ...
  public ArtifactListing selectByArtifactUnitId(long artifactUnitId) {
    try (Connection conn = connectionProvider.getConnection()) {
      return artifactListingDAO
          .selectByArtifactUnitId(conn, artifactUnitId)
          .orElseThrow(
              () ->
                  new RuntimeException(
  ...
  public ArtifactListingPageView selectActiveByActiveStatus(
      long accountId, ArtifactListingFilter filter, int page, int pageSize) {
