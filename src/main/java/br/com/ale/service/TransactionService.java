...
public class TransactionService {
  ...
  public TransactionService(ConnectionProvider connectionProvider) {
  ...
  public void updateStatus(UpdateTransactionRequest request) {
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      if (rowsAffected == 0) {
        throw new RuntimeException("Transaction not found " + "[id=" + request.id() + "]");
      }
  ...
  public List<Transaction> listTransfersByAccount(long accountId) {
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      throw new RuntimeException(
          "Service error while selecting transactions " + "[accountId=" + accountId + "]", e);
    }
  }
}
