...
public class ClientService {
  ...
  private final HashAccountNumberGenerator hashAccountNumberGenerator =
  ...
  public void updateClient(UpdateClientRequest request) {
    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      try {
        ...
        if (rowsAffected == 0) {
          throw new RuntimeException("Client not found [clientId=" + request.id() + "]");
        }
  ...
  public Client getClientByEmail(String email) {
    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      return clientDAO
          ...
          .orElseThrow(() -> new RuntimeException("Client not found [email=" + email + "]"));

    } catch (Exception e) {
      throw new RuntimeException(
          "Service error while selecting client " + "[email=" + email + "]", e);
    }
  }
  ...
  public Optional<Client> getClientByProviderAndId(Provider provider, String providerId) {
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      throw new RuntimeException(
          "Service error while selecting client " + "[providerId=" + providerId + "]", e);
    }
  }
  ...
  public Client getClientById(long clientId) {
    try (Connection conn = connectionProvider.getConnection()) {
      return clientDAO
          .selectById(conn, clientId)
          .orElseThrow(() -> new RuntimeException("Client not found [clientId=" + clientId + "]"));
    } catch (Exception e) {
      throw new RuntimeException(
          "Service error while selecting client " + "[clientId=" + clientId + "]", e);
    }
  }
  ...
  public int activate(long clientId) {
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      throw new RuntimeException(
          "Service error while selecting client to activate " + "[clientId=" + clientId + "]", e);
    }
  }
  ...
  public void deleteClient(long id) {
    ...
    try (Connection conn = connectionProvider.getConnection()) {
      ...
      throw new RuntimeException("Service error while deleting client " + "[id=" + id + "]", e);
    }
  }
}
