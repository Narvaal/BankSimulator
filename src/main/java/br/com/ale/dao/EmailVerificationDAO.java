...
public class EmailVerificationDAO {
    ...
    try (PreparedStatement stmt = conn.prepareStatement(sql, new String[] {"id"})) {
      ...
      if (rowsAffected == 0) {
        throw new RuntimeException(
            "Failed to insert email verification [clientId="
                + request.clientId()
                + ", token="
                + request.token()
                + "]");
  ...
  public Optional<EmailVerification> findValidByToken(
      Connection conn, String token, EmailVerificationType type) {
    ...
    try (PreparedStatement stmt = conn.prepareStatement(sql)) {
      ...
      throw new RuntimeException(
          "Database error while fetching valid email verification [token=" + token + "]", e);
    }
  }

  public Optional<EmailVerification> findActiveByClientId(
      Connection conn, long clientId, EmailVerificationType type) {
    ...
    try (PreparedStatement stmt = conn.prepareStatement(sql)) {
      ...
      throw new RuntimeException(
          "Database error while fetching active email verification [clientId=" + clientId + "]", e);
    }
  }
  ...
  public void markVerified(Connection conn, long id) {
    ...
    try (PreparedStatement stmt = conn.prepareStatement(sql)) {
      ...
      throw new RuntimeException(
          "Database error while marking email verification verified [id=" + id + "]", e);
    }
  }

  public void invalidatePreviousTokens(Connection conn, long clientId, EmailVerificationType type) {
    ...
    try (PreparedStatement stmt = conn.prepareStatement(sql)) {
      ...
      throw new RuntimeException(
          "Database error while invalidating tokens [clientId=" + clientId + "]", e);
    }
  }
