...
public class ArtifactUnitDAO {
    ...
    try (PreparedStatement stmt = conn.prepareStatement(sql, new String[] {"id"})) {
      ...
      if (rowsAffected == 0) {
        throw new RuntimeException(
            "Failed to insert artifactUnit [artifactId="
                + request.artifactId()
                + ", ownerAccountId="
      ...
      try (ResultSet rs = stmt.getGeneratedKeys()) {
        if (!rs.next()) {
          throw new RuntimeException(
              "Failed to retrieve artifactUnit id [artifactId="
                  + request.artifactId()
                  + ", ownerAccountId="
                  + request.ownerAccountId()
                  + "]");
  ...
  public void updateOwner(Connection conn, long artifactId, long ownerAccount) {
    ...
    try (PreparedStatement stmt = conn.prepareStatement(sql)) {
      ...
      throw new RuntimeException(
          "Database error while updating artifact unity owner " + "[artifactId=" + artifactId + "]",
          e);
    }
  }
  ...
  public boolean tryTransferOwnership(
      Connection conn, long id, long sellerAccountId, long buyerAccountId) {
    ...
    try (PreparedStatement stmt = conn.prepareStatement(sql)) {
      ...
      throw new RuntimeException(
          "Failed to transfer artifact unity [id="
              + id
              + ", seller="
              + sellerAccountId
              + ", buyer="
  ...
  public ArtifactUnitPageView selectByOwnerAccount(
      Connection conn, long ownerAccountId, int page, int pageSize) {
    ...
    try (PreparedStatement stmt = conn.prepareStatement(sql)) {
      ...
      try (ResultSet rs = stmt.executeQuery()) {
        ...
        return new ArtifactUnitPageView(items, page, pageSize, totalPages, totalItems);
      }
