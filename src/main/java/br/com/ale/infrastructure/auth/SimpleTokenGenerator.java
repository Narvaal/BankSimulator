...
public class SimpleTokenGenerator implements TokenGenerator {
  ...
  public SimpleTokenGenerator(PrivateKey privateKey, PublicKey publicKey) {
  ...
  public AuthToken generate(long clientId) {
    ...
    String payload =
        clientId + ":" + issuedAt.toEpochMilli() + ":" + expiresAt.toEpochMilli() + ":" + tokenId;

    String signature = SignatureService.sign(payload, privateKey);
    ...
    return new AuthToken(clientId, token, expiresAt);
  }
