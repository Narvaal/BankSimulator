...
public class SchemaInitializer implements ApplicationRunner {
  ...
  public SchemaInitializer(ConnectionProvider connectionProvider, boolean autoCreate) {
  ...
  private boolean tableExists(Connection conn, String tableName) throws Exception {
    ...
    try (ResultSet rs = metaData.getTables(null, schema, normalizedTable, new String[] {"TABLE"})) {
      return rs.next();
    }
  }
  ...
  private String loadSchemaSql() throws Exception {
    try (InputStream schema = getClass().getClassLoader().getResourceAsStream("schema.sql")) {
      if (schema == null) {
