...
public class Client {
  ...
  private String validateName(String name, Provider provider) {
    if (provider == Provider.LOCAL) {
      if (name == null || name.isBlank() || name.length() < 3) {
        throw new IllegalArgumentException("Client name cannot be blank [name=" + name + "]");
      }
    }
    return name;
  }
  ...
  private String validateEmail(String email) {
    if (email == null || email.isBlank()) {
      throw new IllegalArgumentException("Client email cannot be blank [email=" + email + "]");
    }
    return email;
  }
  ...
  private String validatePassword(String password, Provider provider) {
    if (provider == Provider.LOCAL) {
      if (password == null || password.isBlank()) {
        throw new IllegalArgumentException("Password is required for LOCAL provider");
      }
    }
    return password;
  }
  ...
  private String validateProviderId(String providerId, Provider provider) {
    if (provider != Provider.LOCAL) {
      if (providerId == null || providerId.isBlank()) {
        throw new IllegalArgumentException("ProviderId is required for provider " + provider);
      }
    }
    return providerId;
  }
