...
### ADR-018: Frontend Test Suite — 93.4% de linhas + Gate Vitest 90%
**Contexto:** o frontend tinha só 21 testes (utils + parte do ArtifactCard); as páginas inteiras tinham 0. Decisão do usuário: mesmo tratamento do backend — gate de 90% de linhas bloqueando pre-commit e CI.  
**Decisão:** 63 testes novos (84 no total) cobrindo todas as páginas por comportamento com fetch mockado: auth (Login/CreateAccount/ResetPassword/EmailVerification/Google SDK stubado), Marketplace (filtros por URL, debounce, compra/cancelamento), Reward (bundles, claim/cooldown), Home (venda com validação de preço), Search/Profile/Logs/ArtifactDetail/NavBar/UserMenu/Router. Infra reutilizável em `src/test/`: `renderWithProviders` (MemoryRouter + React Query), `mockFetch` com gravação de chamadas, fixtures compatíveis com os DTOs, stubs de jsdom (`setup.ts`). Gate: `thresholds.lines: 90` no `vite.config.ts` (cobertura sobre `src/**` inteiro; exclusão justificada só de `main.tsx`), pre-commit frontend roda `lint + test:coverage`, CI idem antes do build.  
