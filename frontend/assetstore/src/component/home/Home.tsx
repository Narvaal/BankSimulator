...
async function getAssetUnits(
    ...
    const res = await fetch(
        `https://BankSimulator.us-east-2.elasticbeanstalk.com/asset-units?ownerId=${ownerId}&page=${page}&pageSize=${pageSize}`,
        {credentials: "include"}
    );
...
async function postAssetOffer(assetUnityId: number, price: number) {
    const res = await fetch(
        `https://BankSimulator.us-east-2.elasticbeanstalk.com/asset-offers`,
        {
            method: "POST",
            credentials: "include",
            headers: {
